...
- Web 控制台支持实时日志、车牌末次位置查询、多入口选择和动态路径示意。
...
浏览器打开 `http://127.0.0.1:8080/`。Web 端会启动采集/推理线程，实时写入 `vehicle_locator.db`，并提供车牌查询、多入口路线生成、监控证据图和实时通行日志展示。若只想查看现有数据库，可使用 `--backend none`。
...
- 先选择北/西/南/东任一入口，再在查询框输入车牌并回车，页面会显示末次摄像头、时间、车牌裁切图和从所选入口生成的路径示意。
